...
**Reason**: All sub-navigation routes have been removed. The sidebar no longer has expandable items with sub-pages.
**Migration**: Delete route files for: `/orders/drafts`, `/orders/abandoned`, `/products/collections`, `/products/inventory`, `/products/purchase-orders`, `/products/transfers`, `/products/gift-cards`, `/customers/segments`, `/marketing/*`, `/content/*`, `/analytics/reports`, `/analytics/live`.

### Requirement: Secondary navigation routes
**Reason**: Online Store and Settings routes have been removed from the navigation.
**Migration**: Delete route files for `/online-store` and `/settings`. Delete route files for `/discounts` and `/markets`.
